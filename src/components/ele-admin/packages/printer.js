...
export default {
  ...
  print(
    ...
    if (close === undefined && blank && !this.isIE()) {
      close = true;
    }
    ...
    if (horizontal !== undefined) {
      sizeHtml = `size: ${horizontal ? 'landscape' : 'portrait'};`;
    }
    // 页间距设置
    if (margin === 0 || margin) {
      marginHtml = `margin: ${margin};`;
    }
    ...
    if (blank) {
      ...
      // 去除js
      html = html.replace(/<script/g, '<div style="display:none;" ')
  ...
  printHtml(
    ...
    if (blank === undefined && this.isIE() && iePreview) {
      blank = true;
    }
    if (close === undefined && blank && !this.isIE()) {
      close = true;
  ...
  printPage(
    ...
    // 添加页眉页脚
    if (header || footer) {
      html += this.addHeaderFooter(contentHtml, header, footer);
    } else {
      html += contentHtml;
    }
    html += this.getPrintHtml(horizontal, close, print, iePreview, before, done, margin);
   ...
   * 生成控制打印的html
   * @param horizontal 是否横向打印
   * @param close 打印完是否关闭页面
   * @param print 是否立即调起打印
   * @param iePreview 是否支持ie打印预览
   * @param before 打印前回调
   * @param done 打印后回调
   * @param margin 页间距
  ...
  getPrintHtml(horizontal, close, print, iePreview, before, done, margin) {
    ...
    if (iePreview && this.isIE()) {
      ...
      if (print) {
        html += `<script>window.onload=function(){${hideLoadJs}window.WebBrowser.ExecWB?(window.WebBrowser.ExecWB(7,1)):window.print();${closeJs}}</script>`;
      }
    } else if (print) {
    ...
    if (!arraybuffer) {
      ...
      req.onload = () => {
        if ([200, 201].indexOf(req.status) === -1) {
          return error && error(req.status, req.statusText);
        }
  ...
  makeTable(data, cols) {
    ...
    eachCols((i, c) => {
      ...
      colgroup.push('<col');
      if (c.width) {
        colgroup.push(` width="${c.width}"`);
      }
      colgroup.push('/>');
      if (c.width && !/\d+%$/.test(width)) {
        width += (c.width + 1);
      } else {
        needSetWidth = false;
      }
    ...
    let trs = cols.map(col => {
      let ths = col.map(c => `<th colspan="${c.colspan || 1}" rowspan="${c.rowspan || 1}" align="${c.thAlign || c.align || 'left'}" style="${c.thStyle}">${c.title || ''}</th>`);
   ...
   * @param before
   * @param done
   * @returns {{beforeJs: string, taskId: string, doneJs: string}}
   */
  ...
  // 当前窗口打印隐藏的iframe的id
  pFrameId: 'ele-printer-frame',
  // 打印核心样式的style的id
  pStyleId: 'ele-printer-style',
  // 打印方向设置的style的id
  pSetId: 'ele-printer-set',
  // 正在打印标识的class
  printingClass: 'ele-printer-printing',
  // 打印时隐藏的class
  hideClass: 'ele-printer-hide',
  // 打印时隐藏不占位置的class
  hideNoneClass: 'ele-printer-hide-none'
}
